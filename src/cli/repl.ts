      ...
      try {
        ...
        if (beforeTime !== undefined) {
          ...
          if (result.size === 0) {
            ...
            let journeyNumber = 1;
            for (const { departureTime: depTime, result: run } of result) {
              const arrival = run.arrivalAt(toStop.id);
              console.log(
                `\nJourney ${journeyNumber++} — ` +
                  `depart ${timeToString(depTime)}, ` +
                  `arrive ${arrival ? timeToString(arrival.arrival) : '?'} ` +
                  `(${arrival ? arrival.legNumber - 1 : '?'} transfer${arrival?.legNumber === 2 ? '' : 's'}):`,
              );
              const route = run.bestRoute(toStop.id);
              if (route) {
                route
                  .toString()
                  .split('\n')
          ...
          if (arrivalTime === undefined) {
            ...
            console.log(
              `Arriving to ${toStop.name} at ${timeToString(arrivalTime.arrival)} ` +
                `with ${arrivalTime.legNumber - 1} transfer${arrivalTime.legNumber === 2 ? '' : 's'} ` +
                `from ${fromStop.name}.`,
            );
          }
