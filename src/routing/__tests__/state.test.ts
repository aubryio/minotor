...
describe('RoutingState', () => {
  ...
  describe('updateArrival aggregate overwrite rules', () => {
    it('does not overwrite an aggregate arrival with a later arrival from a higher round', () => {
      const state = RoutingState.fromTestData({
        nbStops: 3,
        arrivals: [[1, timeFromHM(8, 30), 1]],
      });

      state.updateArrival(1, timeFromHM(8, 45), 2);

      assert.deepStrictEqual(state.getArrival(1), {
        arrival: timeFromHM(8, 30),
        legNumber: 1,
      });
    });

    it('does not overwrite an aggregate arrival with an equal-time arrival using more legs', () => {
      const state = RoutingState.fromTestData({
        nbStops: 3,
        arrivals: [[1, timeFromHM(8, 30), 2]],
      });

      state.updateArrival(1, timeFromHM(8, 30), 3);
    ...
    it('prefers fewer legs when the arrival time is equal', () => {
      const state = RoutingState.fromTestData({
        nbStops: 3,
        arrivals: [[1, timeFromHM(8, 30), 3]],
      });

      state.updateArrival(1, timeFromHM(8, 30), 2);
    ...
    it('still updates the aggregate arrival when the new arrival is earlier', () => {
      const state = RoutingState.fromTestData({
        nbStops: 3,
        arrivals: [[1, timeFromHM(8, 45), 1]],
      });
