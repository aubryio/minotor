...
describe('GTFS parser', () => {
  ...
  it('should correctly parse timetable from GTFS feed', async () => {
    ...
    assert.strictEqual(route.nbStops(), 2);
    assert.strictEqual(
      route.arrivalAt(beattyAirportId, 0),
      Time.fromHMS(8, 0, 0).toMinutes(),
    );
    assert.strictEqual(
      route.departureFrom(beattyAirportId, 0),
      Time.fromHMS(8, 0, 0).toMinutes(),
    );
    assert.strictEqual(
      route.arrivalAt(bullfrogId, 0),
      Time.fromHMS(8, 10, 0).toMinutes(),
    );
    assert.strictEqual(
      route.departureFrom(bullfrogId, 0),
